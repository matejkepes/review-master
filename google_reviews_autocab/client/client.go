...
func Send(sendURL string, method string, headers map[string]string, params url.Values, jsonBody []byte) string {
	...
	if jsonBody == nil {
		...
		req, _ = http.NewRequest(method, baseURL.String(), bytes.NewReader(jsonBody))
	}
